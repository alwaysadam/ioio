 ...
 *   patternLen - length of bitPattern (sorry, had trouble auto-discovering)
...
int main() {
  led_init();
  char *bitpattern = HEAT_3RD_ON;

  while (1) {
      repeatPattern(bitpattern, sizeof(HEAT_3RD_ON)-1, 0, 1000, 8, 1000);
      DelayMs(5000*1.05);
      repeatPattern(bitpattern, sizeof(HEAT_3RD_OFF)-1, 0, 1000, 8, 1000);
      DelayMs(60000);
  }
  return 0;
...
int repeatPattern(char pattern[], int patternLen, int inversion, int freq, int times, int tdelay) {
    // This function calls bitPattern repeatedly for a fixed number of times with a fixed delay
    int repeat;
    for (repeat = 0; repeat < times; repeat++) {
        bitPattern(pattern, patternLen, inversion, freq);
        if (repeat < times - 1) { DelayMs(tdelay*CLK_SCALE);}
    }
}

int bitPattern(char pattern[], int patternLen, int inversion, int freq) {
